...
public class BlockModelProvider extends FabricModelProvider {
     ...
     * this: {@code BlockModelGenerators.createRotatedVariantBlock} only ever varies y, so the three strain resins
     * were generating 4 variants and tiling far more obviously than the normal strain beside them.
     * <p>
     * Generated rather than hand-authored on purpose. A manual copy in resources would collide with the datagen
     * output on the same resource path - the same duplicate-root problem that had 120 files disagreeing - because
     * the model and the blockstate come out of one vanilla call and cannot be split apart.
     * <p>
     * The block item model is unaffected: vanilla emits {@code {"parent": <block model>}} for BlockItems at the end
     * of its own run, not from this call.
     */
