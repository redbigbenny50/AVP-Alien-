...
public class ResinContainerBlockEntity extends RandomizableContainerBlockEntity {
    ...
    /**
     * ⭐⭐ THE FIX FOR "resin containers lose their contents when you relog". Mirrors ChestBlockEntity exactly.
     * <p>
     * ⚠⚠ RandomizableContainerBlockEntity DOES NOT SERIALISE THE ITEM LIST - it only adds loot-table plumbing on top of
     * BaseContainerBlockEntity, which writes the custom NAME and nothing else. ChestBlockEntity carries its own items
     * field AND its own pair of overrides, and so must every subclass. Without these the contents lived in memory only
     * and died on chunk unload.
     * </p>
     * <p>
     * ⚠ tryLoadLootTable / trySaveLootTable FIRST in each: that stops an unopened loot container being overwritten with
     * an empty inventory, or re-rolling its loot on every load.
     * </p>
     */
    @Override
