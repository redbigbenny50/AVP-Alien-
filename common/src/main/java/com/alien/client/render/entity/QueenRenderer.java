...
public class QueenRenderer extends AzEntityRenderer<Queen> {
    ...
    /**
     * Per-bone texture override: the human-made attachments bolted onto her are NOT part of the queen's own
     * texture, so each draws from its own sheet. Everything else returns null and falls through to the body
     * texture (which is variant-aware - normal/nether/aberrant/irradiated).
     * <p>
     * Visibility is not decided here: {@code QueenAnimator} already hides these bones until their chain / device
     * is actually attached. This only says what they are painted with when shown.
     */
